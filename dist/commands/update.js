...
import { downloadManifest, downloadFile } from '../sync/download.js';
...
import { getRelativePath, validateFrontmatter, writeFileAtomic, createBackup, } from '../sync/copy.js';
import { readState, writeState } from '../state/state.js';
import { errorUnknownKind, errorFrontmatterInvalid, } from '../errors.js';
import { formatSummary, formatDiff, formatError, formatWarning } from '../ui/output.js';
export async function runUpdate(options = {}) {
    const { force = false, fetcher, claudeDir, confirmFn = defaultConfirm, logFn = console.log, } = options;
    const output = [];
    const log = (msg) => {
        output.push(msg);
        logFn(msg);
    ...
    try {
        ...
        const overriddenReadFile = async (path) => {
        ...
        for (const installedPath of installedFiles) {
            const stillInManifest = manifest.entries.some((e) => getRelativePath(e.kind, e.source) === installedPath);
            if (!stillInManifest) {
        ...
        for (const diffEntry of diffEntries) {
            ...
            if (diffEntry.category === '-') {
                ...
                try {
                    ...
                    if (idx >= 0)
                        newInstalledFiles.splice(idx, 1);
                }
                catch { }
                continue;
            }
