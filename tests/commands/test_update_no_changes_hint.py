...
def test_no_changes_message_includes_last_sync_date(tmp_path):
    ...
    out = io.StringIO()
    run_update(clone_dir=clone, dest_dir=dest_dir, state_path=state_file, out=out, stdin=io.StringIO(""), skip_pull=True)

    output = out.getvalue()
    # Should show date in readable form
